...
const BirthHeader: React.FC<BirthHeaderProps> = ({
  showLoginLink,
  className,
  guests,
}) => {
  ...
  return (
    <>
      <header
        className={`w-full fixed top-0 left-0 p-6 flex flex-wrap items-center justify-between gap-4 ${className}`}
      >
        ...
        {showLoginLink && (
          <div className="text-white text-base font-bold">
            <div>Já confirmou sua presença anteriormente?</div>
            <div
              onClick={() => setIsModalOpen(true)}
              className="px-4 py-2 rounded-md bg-purple hover:opacity-90 mt-2 text-white text-center hover:underline hover:cursor-pointer"
            >
              Acessar Dashboard
            </div>
          </div>
      ...
      >
        <p>
          Para acessar as informações após confirmação da presença, preencha o
          seu nome ou apelido que foi utilizado na confirmação!
        </p>
        <input
          id="guest"
          type="text"
