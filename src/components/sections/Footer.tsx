  ...
  return (
    <footer className="container flex flex-col justify-center items-center my-4 gap-4 md:my-12 md:gap-12 text-black dark:text-white">
      <div className='w-full flex justify-center items-center mx-auto'>
        ...
        <div className='text-[40px] px-14'><i className={IconType.LOGO} /></div>
          ...
          {socials.map(({ icon, label, url }) => (
            <li key={label}>
              <a href={url} target="_blank" rel="noopener noreferrer">
                <div className='flex gap-1 justify-start items-center transition duration-500 hover:text-primary' title={label} aria-label={label}>
                  <i className={icon} />
                  <span>
                    {label}
                  </span>
